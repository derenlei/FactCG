...
class Inferencer():
    ...
    def batch_inference(self, premise: list, hypo: list):
        ...
        output_score_all = []
        best_chunk_ids = []
        output_score_per_example = torch.split(output_score_all_flat, [
                                               # , dim=1)
                                               math.prod(n) for n in chunksize_per_example])
        for output_score, chunk_size in zip(output_score_per_example, chunksize_per_example):
            ...
            output_score_val = output_score.view(hypo_len, premise_len).max(
                # sum or mean depends on the task/aspect
                dim=1).values.mean().item()
    ...
    def chunk_src_minicheck(self, src, max_chunk_size=500):
        ...
        src_chunks = [chunk.replace(" \n ", '\n').strip()
                      for chunk in chunks(src_sents, max_chunk_size)]
    ...
    def tokenize(self, premise_list, hypo_list):
        if "t5" in self.model_name.lower() or "deberta" in self.model_name.lower():
            ...
            try:
                output = self.tokenizer(premise_list, hypo_list, truncation='only_first',
                                        padding='longest', max_length=2048, return_tensors='pt')
            except:
                warning('text_b too long...')
                output = self.tokenizer(premise_list, hypo_list, truncation=True,
                                        padding='longest', max_length=2048, return_tensors='pt')
